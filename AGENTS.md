...
Run `pnpm lint` after changes and fix every error. `oxlint.config.ts` registers `@shadcn/lint` and enforces all six rules as errors: component contracts, known Tailwind classes, static component class names, semantic colors, theme or scale values, and class-based styling. Use CSS custom properties for runtime geometry and named theme tokens for custom values. Shared primitives in `packages/ui/src/components` retain the upstream shadcn Base Nova APIs and own their internal styles. Product adapters belong in `packages/ui/src/compositions`; use `FormFieldError` there for arbitrary validator values. Import `buttonVariants` from the button module and `toast` directly from Sonner. Keep semantic headings inside CardTitle at call sites. Run `pnpm check:shadcn` to detect source or CSS-contract drift. The `utils.test.ts` allowance for `foo`, `bar`, and `baz` exercises opaque class merging; those names are not production CSS. Add component variants for appearance changes and retain layout classes at call sites.
